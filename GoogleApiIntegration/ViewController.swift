...
extension ViewController {
    @IBAction func searchByAddress(_ sender: UIBarButtonItem) {
        ...
        let findAction = UIAlertAction(title: "Find Address", style: UIAlertActionStyle.default, handler: { [weak self] (UIAlertAction) -> Void  in
        ...
        let closeAction = UIAlertAction(title: "Close", style: UIAlertActionStyle.cancel, handler: { [weak self] (UIAlertAction) -> Void  in
        })
    ...
    @IBAction func searchByLatAndLong(_ sender: UIBarButtonItem) {
        ...
        let findAction = UIAlertAction(title: "Find Address", style: UIAlertActionStyle.default, handler: { [weak self] (UIAlertAction) -> Void in
            ...
            if latitude != nil && longitude != nil {
                strongSelf.viewMap.camera = GMSCameraPosition.camera(withLatitude: latitude!, longitude: longitude!, zoom: 14.0)
                let title = "Lat: " + (latitude?.description)! + " Long: " + (longitude?.description)!
                strongSelf.setuplocationMarker(coordinate: CLLocationCoordinate2D(latitude: latitude!, longitude: longitude!), title: title)
            }
        })
